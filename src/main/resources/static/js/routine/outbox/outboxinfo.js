layui.extend({
	...
	$(".layui-icon-rate").click(function(){
		...
		$.ajax({
				...
				dataType: "json",
    			data:{"mailIds":mailId,"starStatus":state},
	...
	$("#zf").click(function(){
		...
		$.ajax({
    			type:"post",
    			url:layui.setter.project + "/outbox/getOutboxInfo",
    			async:true,
				xhrFields: {
					withCredentials: true
				},
    			...
    			success:function(result){
    				...
    				$.each(result.mailUsers, function(index, value) {
    					...
    					if(value.status==1){
    						...
    						muser = muser+ value.userss.userName + "、";
    					}
					});
    				if(suser1!=""){
    				...
    				if(muser!=""){//密送人不等空
    					muserFlag = true;
    					muser = muser.substring(0, muser.length - 1);
    				}
