layui.extend({
 ...
 //关闭粘贴样式的过滤
	...
	let arr1;//抄送人
	if($("#state").val()==1){//回复
		arr = JSON.parse('[' + data.senderUser + ']');//收件人
		$("#zhuti").val("Re："+data.mailTheme);
		//$("#div1").val(data.mailContent);
		var contents = $("#contents").val();
		editor.txt.html("</br></br></br></br></br></br>------------------原始邮件------------------</br></br>"+contents+data.mailContent);
	}else if($("#state").val()==2){//回复全部
		arr=JSON.parse('[' + $("#suserIds").val() +','+data.senderUser+']');
